...
with dates as (select distinct(year_month) from calendar),
    ...
    users_active as (select u.user_id, date(policy_start_date) active_date
                            from users u left join policy p on u.user_id = p.user_id
                     UNION
                     select u.user_id, date(policy_end_date) active_date
                            from users u left join policy p on u.user_id = p.user_id),
    ...
    dates_table_filter as (select *,
                ROW_NUMBER () over ( partition by user_id, year_month order by current desc, active_date desc) row_num
                from base_dates_table),
